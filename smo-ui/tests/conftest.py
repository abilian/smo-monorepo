...
class MockClusterService:
    def fetch_clusters(self) -> list[dict]:
        return [
            {
                "name": "cluster-1", "availability": True, "location": "us-east-1",
                "acceleration": True, "available_cpu": 12.0, "available_ram": "48.00 GiB",
                "grafana": "http://grafana.test/d/cluster-1"
            },
            {
                "name": "cluster-2", "availability": False, "location": "eu-west-1",
                "acceleration": False, "available_cpu": 20.0, "available_ram": "90.00 GiB",
...
class MockGraphService:
    ...
    def fetch_graph(self, name: str) -> Graph | None:
        if name == "graph-1":
            return Graph(
                name="graph-1", project="default", status="Running",
